...
from constants import SELECTORS, WHITE_HEX, WHITE_HEX_SHORT
...
class TestHamburgerIcon:
    ...
    def test_hamburger_icon_uses_css_variable_dark_mode(self, page: Page, jekyll_server: str):
        ...
        bg = toggle_before.evaluate("el => getComputedStyle(el, '::before').backgroundImage")

        # Should have an SVG background (contains 'url' and 'svg')
        assert "url" in bg.lower(), "Should have background image URL"

    def test_hamburger_icon_uses_css_variable_light_mode(self, page: Page, jekyll_server: str):
    ...
    def test_hamburger_icon_color_differs_by_theme(self, page: Page, jekyll_server: str):
        ...
        page.reload()
        toggle = page.locator(SELECTORS["sidebar_toggle"])
        dark_bg = toggle.evaluate("el => getComputedStyle(el, '::before').backgroundImage")

        # Get light mode icon
        page.evaluate("() => { localStorage.setItem('theme', 'light'); }")
        page.reload()
        light_bg = toggle.evaluate("el => getComputedStyle(el, '::before').backgroundImage")
    ...
    def test_hamburger_icon_white_when_menu_open_light_mode(self, page: Page, jekyll_server: str):
        ...
        toggle = page.locator(SELECTORS["sidebar_toggle"])
        closed_icon = toggle.evaluate("el => getComputedStyle(el, '::before').backgroundImage")

        # Open the sidebar
        toggle.click()

        # Get icon after opening menu (should be white: #ffffff)
        open_icon = toggle.evaluate("el => getComputedStyle(el, '::before').backgroundImage")
    ...
    def test_hamburger_icon_white_when_menu_open_dark_mode(self, page: Page, jekyll_server: str):
        ...
        assert self._has_white_color(closed_icon), "Closed menu icon should be white in dark mode"
        assert self._has_white_color(open_icon), "Open menu icon should be white in dark mode"

    def test_hamburger_outline_thickness_when_open(self, page: Page, jekyll_server: str):
